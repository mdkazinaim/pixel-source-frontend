...
export const STOCK_SITES: StockSite[] = [
  ...
  },
  {
    ...
    getUrl: (term, cat) => {
      if (cat === "icons") return `https://www.freepik.com/search?format=search&last_filter=type&last_value=icon&query=${term}&type=icon`;
      if (cat === "videos") return `https://www.freepik.com/search?format=search&last_filter=type&last_value=video&query=${term}&type=video`;
      return `https://www.freepik.com/search?format=search&term=${term}`;
    },
    icon: (
  ...
  },
  {
    ...
    name: "Magnific",
    categories: ["images"],
    getUrl: (term) => `https://www.magnific.com/search?format=search&term=${term}`,
    icon: (
